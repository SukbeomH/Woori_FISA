...
def closest_station_info(lat: float, lon: float) -> dict:
    ...
    df = df.dropna(subset=["LAT", "LOT"])  # LAT와 LOT 열에서 NaN 값 제거
    df = df[
        df["LAT"].apply(lambda x: x.replace(".", "", 1).isdigit())
    ]  # LAT 열에서 숫자가 아닌 값 제거
    df = df[
        df["LOT"].apply(lambda x: x.replace(".", "", 1).isdigit())
    ]  # LOT 열에서 숫자가 아닌 값 제거
    df["distance"] = df.apply(
        lambda x: get_distance(lat, lon, x["LAT"], x["LOT"]), axis=1
    )
    return df.loc[df["distance"].idxmin()].to_dict()
