...
# 관객수 100만을 넘긴 영화의 감독과 해당 관객의 영화를 본 관객수를 연도별, 감독별로 구하는 쿼리를 작성하세요.
SELECT
	director, YEAR(release_date), SUM(audience_num)
FROM 
	box_office
WHERE 
	audience_num > 1000000 AND YEAR(release_date) >= 2015
GROUP BY 
	director, YEAR(release_date)
HAVING
	COUNT(director) > 1;
...
SELECT 
	MONTH(release_date) AS 월별,
    movie_type AS 유형별,
    SUM(sale_amt) AS "매출액 소계"
FROM
	box_office
WHERE
	QUARTER(release_date) = 1 AND YEAR(release_date) = 2019
GROUP BY
	MONTH(release_date), movie_type
HAVING
	SUM(sale_amt) > 10000000;
